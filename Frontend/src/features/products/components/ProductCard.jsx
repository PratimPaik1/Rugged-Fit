...
const ProductCard = ({ product, onDelete, onEdit }) => {
  const { _id, title, price, images, category, variants } = product;

  const navigate = useNavigate();
  const mainImage = images?.[0]?.url || 'https://via.placeholder.com/400';

  return (
    <div className="card card-hover p-0 overflow-hidden flex flex-col h-full relative group" onClick={() => navigate(`/dashboard/seller-products/${_id}`)}>
      {/* Visual Component */}
        ...
        <div className="mt-4 pt-4 border-t border-[var(--border-subtle)] flex items-end justify-between">
          <div className="flex flex-col">
            ...
            <span className="text-lg font-bold text-[var(--text-primary)] leading-none">
              ...
              {(variants[0].sellingPrice || price.amount).toLocaleString()}
            </span>
          </div>
